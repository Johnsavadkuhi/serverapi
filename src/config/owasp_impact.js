module.exports = module.exports = [
  ...
  }, 
  {
    ...
    "impact": "دسترسی غیرمجاز به آبجکت‌های (اشیاء) سایر کاربران می‌تواند منجر به افشای داده‌ها برای افراد غیرمجاز، از دست رفتن داده‌ها یا دستکاری آن‌ها شود. تحت شرایطی خاص، دسترسی غیرمجاز به آبجکت‌ها حتی می‌تواند به تصاحب کامل حساب کاربری منجر شود. ", 
    "exploit": "" , 
    "exploit_fa": "مهاجمان می‌توانند از پایانه‌های رابط برنامه‌نویسی (API) که در برابر نقص احراز هویت در سطح شیء آسیب‌پذیر هستند، با دستکاری شناسهٔ (ID) شیء‌ای که درون درخواست ارسال می‌شود، سوءاستفاده کنند. شناسه‌های اشیاء می‌توانند هر چیزی باشند؛ از اعداد صحیح ترتیبی، شناسه‌های یکتای جهانی (UUID)، یا رشته‌های متنی عمومی. صرف‌نظر از نوع داده، این شناسه‌ها به راحتی در هدف درخواست (پارامترهای مسیر یا رشتهٔ پرس‌وجو)، سرآیندهای درخواست، یا حتی به عنوان بخشی از بدنهٔ درخواست قابل شناسایی هستند.", 
    "solution": "" , 
  }, 
   {
    ...
    "impact": "مهاجمان می‌توانند کنترل کامل حساب‌های سایر کاربران در سیستم را به دست بگیرند، اطلاعات شخصی آن‌ها را بخوانند و از طرف آن‌ها اقدامات حساس انجام دهند. همچنین، سیستم‌ها به احتمال زیاد قادر به تشخیص اقدامات مهاجمان از اقدامات کاربران قانونی نخواهند بود." , 
    "exploit": "" , 
    "exploit_fa": "سازوکار احراز هویت هدف آسانی برای مهاجمان است، چرا که در معرض دید همگان قرار دارد. اگرچه برای بهره‌برداری از برخی مشکلات احراز هویت ممکن است به مهارت‌های فنی پیشرفته‌تری نیاز باشد، اما ابزارهای بهره‌برداری معمولاً به راحتی در دسترس هستند.", 
    "solution": "" , 
  }, 
   {
    ...
    "impact": "دسترسی غیرمجاز به ویژگی‌های خصوصی/حساس یک شیء می‌تواند منجر به افشای داده‌ها، از دست رفتن داده‌ها یا خرابی/فساد داده‌ها شود. تحت شرایطی خاص، دسترسی غیرمجاز به ویژگی‌های اشیاء می‌تواند به افزایش سطح دسترسی یا تصاحب جزئی/کامل حساب کاربری منجر شود." , 
    "exploit": "" , 
    "exploit_fa": "رابط‌های برنامه‌نویسی (API) معمولاً پایانه‌هایی را در معرض دید قرار می‌دهند که تمام ویژگی‌های یک شیء را برمی‌گردانند. این موضوع به‌ویژه در مورد رابط‌های REST صادق است. برای پروتکل‌های دیگر مانند GraphQL، ممکن است به درخواست‌هایی نیاز باشد که با دقت طراحی شده‌اند تا مشخص کنند کدام ویژگی‌ها باید بازگردانده شوند. شناسایی این ویژگی‌های اضافی که می‌توان آن‌ها را دستکاری کرد، به تلاش بیشتری نیاز دارد، اما چند ابزار خودکار نیز برای کمک به این کار در دسترس هستند.", 
    "solution": "" , 
  }, 
   {
    ...
    "impact": "بهره‌برداری (سوءاستفاده) از این آسیب‌پذیری می‌تواند به دلیل اتمام منابع، منجر به اختلال در سرویس‌دهی (DoS) شود. همچنین می‌تواند موجب افزایش هزینه‌های عملیاتی گردد؛ از جمله هزینه‌های مربوط به زیرساخت به دلیل افزایش تقاضای پردازنده (CPU)، افزایش نیاز به فضای ذخیره‌سازی ابری و موارد مشابه." , 
    "exploit": "" , 
    "exploit_fa": "بهره‌برداری از این آسیب‌پذیری نیازمند درخواست‌های سادهٔ رابط برنامه‌نویسی (API) است. می‌توان چندین درخواست هم‌زمان را از یک رایانهٔ شخصی منفرد یا با استفاده از منابع رایانش ابری اجرا کرد. بیشتر ابزارهای خودکاری که در دسترس هستند، به گونه‌ای طراحی شده‌اند که از طریق حجم بالای ترافیک، باعث اختلال در سرویس‌دهی (DoS) شده و نرخ سرویس‌دهی رابط‌های برنامه‌نویسی را تحت تأثیر قرار دهند.", 
    "solution": "" , 
  }, 
   {
    ...
    "impact": "چنین نقص‌هایی به مهاجمان اجازه می‌دهد تا به عملکردهای غیرمجاز دسترسی پیدا کنند. عملکردهای مدیریتی (ادمین) اهداف کلیدی این نوع حمله هستند و ممکن است منجر به افشای داده‌ها، از دست رفتن داده‌ها یا خرابی داده‌ها شوند. در نهایت، این حمله می‌تواند به اختلال در سرویس‌دهی منجر شود." , 
    "exploit": "" , 
    "exploit_fa": "بهره‌برداری از این آسیب‌پذیری مستلزم آن است که مهاجم، فراخوانی‌های قانونی رابط برنامه‌نویسی (API) را به پایانه‌ای ارسال کند که به عنوان یک کاربر ناشناس یا کاربر عادی و فاقد دسترسی ویژه، نباید به آن دسترسی داشته باشد. پایانه‌هایی که در معرض دید قرار گرفته‌اند، به راحتی مورد سوءاستفاده قرار خواهند گرفت.", 
    "solution": "" , 
  }, 
   {
    ...
    "impact": "به طور کلی، تأثیر فنی (مستقیم) قابل توجهی از این حمله انتظار نمی‌رود. با این حال، بهره‌برداری از آن می‌تواند به روش‌های مختلفی به کسب‌وکار آسیب بزند؛ برای مثال: می‌تواند مانع خرید محصول توسط کاربران قانونی شود، یا منجر به تورم در اقتصاد داخلی یک بازی گردد." , 
    "exploit": "" , 
    "exploit_fa": "بهره‌برداری از این آسیب‌پذیری معمولاً شامل درک مدل کسب‌وکاری است که رابط برنامه‌نویسی (API) از آن پشتیبانی می‌کند، یافتن جریان‌های حساس کسب‌وکار، و خودکارسازی دسترسی به این جریان‌ها می‌شود که در نهایت به کسب‌وکار آسیب می‌زند.", 
    "solution": "" , 
  }, 
   {
    ...
    "impact": "بهره‌برداری موفق از این آسیب‌پذیری می‌تواند منجر به شناسایی سرویس‌های داخلی (مانند پویش پورت)، افشای اطلاعات، یا دور زدن فایروال‌ها و سایر سازوکارهای امنیتی شود. در برخی موارد نیز می‌تواند به اختلال در سرویس‌دهی (DoS) یا استفاده از سرور به عنوان یک پیشکار (پروکسی) برای مخفی‌سازی فعالیت‌های خرابکارانه منجر شود." , 
    "exploit": "" , 
    "exploit_fa": "بهره‌برداری از این آسیب‌پذیری مستلزم آن است که مهاجم پایانه‌ای از رابط برنامه‌نویسی (API) را پیدا کند که به یک نشانی اینترنتی (URI) ارائه‌شده توسط سرویس‌گیرنده (کاربر) دسترسی پیدا می‌کند. به طور کلی، حملات SSRF ساده (زمانی که پاسخ درخواست به مهاجم بازگردانده می‌شود)، راحت‌تر از حملات SSRF کور قابل بهره‌برداری هستند؛ در نوع کور، مهاجم هیچ بازخوردی مبنی بر موفقیت یا عدم موفقیت حمله دریافت نمی‌کند.", 
    "solution": "" , 
  }, 
   {
    ...
    "impact": "پیکربندی‌های نادرست امنیتی نه تنها داده‌های حساس کاربران را افشا می‌کنند، بلکه جزئیات سیستمی را نیز فاش می‌سازند که می‌تواند به تسخیر کامل سرور منجر شود." , 
    "exploit": "" , 
    "exploit_fa": "مهاجمان اغلب تلاش می‌کنند تا نقص‌های وصله‌نشده، پایانه‌های رایج، سرویس‌هایی که با پیکربندی‌های پیش‌فرض ناامن در حال اجرا هستند، یا فایل‌ها و پوشه‌های محافظت‌نشده را بیابند تا به دسترسی غیرمجاز یا اطلاعاتی از سیستم دست پیدا کنند. بیشتر این موارد، دانشی عمومی هستند و ممکن است ابزارهای بهره‌برداری از آن‌ها نیز در دسترس باشد.", 
    "solution": "" , 
  }, 
   {
    ...
    "impact": "مهاجمان می‌توانند به داده‌های حساس دسترسی پیدا کنند یا حتی سرور را به طور کامل تسخیر نمایند. گاهی اوقات نسخه‌ها یا استقرارهای مختلف یک رابط برنامه‌نویسی (API) به همان پایگاه داده‌ای متصل هستند که حاوی اطلاعات واقعی است. عوامل تهدید ممکن است از پایانه‌های منسوخ‌شده‌ای که در نسخه‌های قدیمی آن رابط در دسترس باقی مانده‌اند، برای دسترسی به عملکردهای مدیریتی یا بهره‌برداری از آسیب‌پذیری‌های شناخته‌شده سوءاستفاده کنند." , 
    "exploit": "" , 
    "exploit_fa": "عوامل تهدید معمولاً از طریق نسخه‌های قدیمی رابط برنامه‌نویسی (API) یا پایانه‌هایی که بدون وصله‌های امنیتی و با الزامات امنیتی ضعیف‌تر همچنان در حال اجرا رها شده‌اند، به دسترسی غیرمجاز دست می‌یابند. در برخی موارد، ابزارهای بهره‌برداری نیز در دسترس هستند. از سوی دیگر، آن‌ها ممکن است از طریق یک شخص ثالث که اساساً دلیلی برای به اشتراک‌گذاری داده با او وجود ندارد، به اطلاعات حساس دسترسی پیدا کنند.", 
    "solution": "" , 
  }, 
   {
    ...
    "impact": "دامنهٔ تأثیر (این حمله) بسته به این که رابط برنامه‌نویسی (API) هدف، با داده‌های دریافت‌شده چه می‌کند، متفاوت است. بهره‌برداری موفق از این آسیب‌پذیری می‌تواند منجر به افشای اطلاعات حساس برای افراد غیرمجاز، انواع گوناگونی از حملات تزریقی، یا اختلال در سرویس‌دهی شود." , 
    "exploit": "" , 
    "exploit_fa": "بهره‌برداری از این مشکل مستلزم آن است که مهاجمان، سایر رابط‌های برنامه‌نویسی (API) یا سرویس‌هایی را که API هدف با آن‌ها یکپارچه شده است، شناسایی کرده و به طور بالقوه آن‌ها را نیز مورد نفوذ قرار دهند. معمولاً این اطلاعات به صورت عمومی در دسترس نیست، یا API/سرویس یکپارچه‌شده به راحتی قابل بهره‌برداری نمی‌باشد.", 
    "solution": "" , 
  }
